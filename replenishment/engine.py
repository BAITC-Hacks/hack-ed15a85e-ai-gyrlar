    ...
    for i, m in enumerate(months):
        ...
        if remove_outliers and len(positive) >= 6:
            ...
            if value > max(6*center, center+8*1.4826*mad):
                adjusted[i] = center*season[int(m[5:])-1]
                excluded[i] += cleaned[i]-adjusted[i]
    available_rates = []
    for i, m in enumerate(months):
...
def calculate(item, season, settings):
    ...
    growth = 0.0
    if settings.get('trend', True) and len(active) >= 6:
    ...
    lead = int(category_policy.get('lead_days', settings.get('lead_days', 30)))
    review = int(settings.get('review_days', 14))
    safety_days = int(category_policy.get('safety_days', settings.get('safety_days', 7)))
    horizon = lead+review
    ...
    stale = bool(stock_date and (as_of-date.fromisoformat(stock_date)).days > 3)
    ...
    return dict(id=item['id'], supplier=item['supplier'], code=item['code'], article=item.get('article', ''),
                ...
                shortage_days=shortage, lead_days=lead, horizon_days=horizon,
